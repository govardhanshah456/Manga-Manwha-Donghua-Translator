...
from utils.imgproc_utils import rotate_polygons, union_area, xywh2xyxypoly
...
def sort_textblk_list(blk_list: List[TextBlock], im_w: int, im_h: int) -> List[TextBlock]:
    ...
    grid_weights = grid_indices * img_area + 1.2 * (center_x - grid_x * im_w / num_gridx) + (
...
def group_output(blks, lines, im_w, im_h, mask=None, sort_blklist=True) -> List[TextBlock]:
    ...
    for blk in blk_list:
        ...
        if not textblock_splitted:
            for sub_blk in sub_blk_list:
                sub_blk.adjust_bbox(with_bbox=True)
    ...
    for blk in final_blk_list:
        if blk.language == "eng" and not blk.vertical:
            ...
            line_pts = blk.lines_array() + shifted_vec
            line_pts[..., 0] = np.clip(line_pts[..., 0], 0, im_w - 1)
            line_pts[..., 1] = np.clip(line_pts[..., 1], 0, im_h - 1)
            blk.lines = line_pts.astype(np.int64).tolist()
